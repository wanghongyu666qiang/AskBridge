...
DesktopPwaLauncher
  ...
  → 没有可验证 UI Automation 证据时进入剪贴板人工兜底，不猜测控件
...
专用 Chrome 在可验证时将截图和问题放入正确输入区
桌面 PWA 在没有可靠控件证据时打开人工兜底，由用户粘贴而不猜测
并默认停留在用户确认发送的状态
```
...
通用授权规则：未被当前指令明确连续授权的 Phase，完成并通过本阶段验收后必须停止，汇报阶段检查点并等待下一轮授权。Phase 5 已完成实现和真实验收；当前第 31 节已根据用户在 2026-08-03、2026-08-04 和 2026-08-09 的明确接续指令继续实施和验收 Phase 6，不授权进入 Phase 7。
...
Phase 0–6 的历史阶段授权已经完成。用户于 2026-08-10 明确要求“完成整个项目的开发和验收”，当前连续授权覆盖 Phase 7、Phase 8 及 1.0 发布收尾，但仍不授权自动发送、浏览器扩展、模型 API 或网络下发规则。无法由当前单屏机器或未登录账号完成的外部矩阵必须如实列为待验收或由用户明确豁免，不得伪报通过。
...
仓库 main 与 origin/main 的历史提交基线为 Phase 0–4；当前未提交工作区已连续实现 Phase 5–8 候选。
用户于 2026-08-10 明确授权完成整个项目的开发和验收，包括 Phase 7、Phase 8 与 1.0 收尾。
ADR 0001 继续有效：ChatGPT 优先启动现有桌面 PWA 并复用其登录会话。

当前收尾范围：
1. 修复审查发现的请求保留、逐任务取消、配置原子替换、附件完成回执、运行时设置刷新和安装器一致性；
2. 完成真实 Windows 设置、热键、截图、安装脚本和生成的 Setup EXE 验收；
3. 四家真实供应商必须实际插入固定非敏感测试文本才算通过，登录页不得当作成功；
4. 完成桌面进程与专用 Chrome 性能证据，并记录快捷键到 UI 的延迟；
5. 同步 README、隐私、故障排查、发布清单、验收报告和交接文档；
6. 外部 Windows/显示器/DPI/主题矩阵及最终产物目录需要用户提供或明确豁免/选择；
7. 全部适用证据闭环后才标记 `1.0.0` 并生成最终包。

仍不得实现自动发送、浏览器扩展、模型 API 或网络下发规则。
...
4. 构建命令；
5. 测试与构建结果；
6. 四家入口、文字/图片准备、安全兜底、安装和性能的真实验收结果；
7. 尚未完成内容；
8. 外部矩阵、最终产物位置及任何豁免。

自动检查通过后执行当前任务范围内的手工验收。涉及启动桌面程序、
占用全局快捷键、打开桌面 PWA 或专用 Chrome、修改剪贴板时先说明影响。
最终包必须保存到用户明确给出的绝对目录，不得默认写入 C 盘或其他位置。
```
...
桌面端通过可自定义快捷键完成截图或文字采集；需要投递时，按供应商选择现有桌面 PWA 或与日常浏览器隔离的 AskBridge 专用 Chrome。Phase 5 分别通过 UI Automation/剪贴板 adapter 或 CDP adapter，将内容可靠地放入用户选择的 AI 网页；Phase 6 在同一 `prepare(...)` seam 内增加四家内置规则、登录识别和通用回退。
