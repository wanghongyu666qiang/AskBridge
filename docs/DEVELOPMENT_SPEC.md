...
通用授权规则：未被当前指令明确连续授权的 Phase，完成并通过本阶段验收后必须停止，汇报阶段检查点并等待下一轮授权。当前第 31 节只授权校正并验收已经存在的 Phase 0–2 工作，不授权进入 Phase 3。
...
当前第 31 节授权对已经存在的 Phase 0–2 实现做一次跨阶段基线校正。该授权只用于消除 v2.1 与现有代码、测试和文档之间的差异，不把 Phase 0 或 Phase 1 重新标记为未完成，也不授权开始 Phase 3。校正后必须执行完整 Phase 2 检查点并停止。
...
仓库 main 与 origin/main 当前停在 Phase 0/1 的已推送提交 4ada778。
工作树中存在尚未提交的 Phase 2 实现。继续工作时先保留用户已有修改，
然后让现有 Phase 0–2 与本文档 v2.1 对齐。

本轮只允许：
1. 将配置迁移到 schema v3，并采用编译时内置供应商加用户覆盖的模型；
2. 固定 1.0 的 auto_submit 为 false；
3. 按第 18.1.1 节修正快捷键候选 ID、注销失败回滚和 ID 回收；
4. 让 CapturedImage 保留 RGBA，保留独立的内存 PNG 编码能力；
5. 移除截图成功路径中的剪贴板副作用；
6. 同步 README、Phase 2 计划和交接文档；
7. 补齐自动化测试并完成 Phase 2 构建检查点。

不得开始问题输入框、专用 Chrome、CDP、网页适配器或自动投递。
不得实现浏览器扩展。正式程序不得启动本地 HTTP 服务。
...
占用全局快捷键或人工框选时先说明影响。Phase 2 验收完成后停止，
不得开始 Phase 3。
```
